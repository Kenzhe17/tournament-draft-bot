...
class RiddlesView(discord.ui.View):
    """View для Загадки."""
    ...
    @discord.ui.button(label="Ответить", style=discord.ButtonStyle.primary, custom_id="riddle_answer")
    async def answer_button(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.send_modal(RiddleAnswerModal(self.guild_id, self.user_id, self.bet, self.question, self.answer, self.hints, self.attempts, self.hint_index))

    @discord.ui.button(label="💡 Подсказка", style=discord.ButtonStyle.secondary, custom_id="riddle_hint")
    async def hint_button(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if self.hint_index < len(self.hints):
            ...
            # Обновить view
            view = RiddlesView(self.guild_id, self.user_id, self.bet, self.question, self.answer, self.hints)
...
class RiddleAnswerModal(discord.ui.Modal, title="Ваш ответ"):

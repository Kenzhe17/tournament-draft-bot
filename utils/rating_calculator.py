...
def get_loss_reduction(current_elo: int) -> float:
    """
    Get loss reduction factor for weak players (less penalty for losses).

    Args:
        current_elo: Current player ELO

    Returns:
        Reduction factor (1.0 = full penalty, <1.0 = reduced penalty)
    """
    if current_elo < 1100:
